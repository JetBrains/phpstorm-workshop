...
// 4. Using Resume, execute until we are on the next breakpoint. Let's step into the getAll() function using the keyboard
//        shortcut.
//    We are now in the FruitRepository and can see all variables in it.
// 5. Using Step Out, step back to where we were.
// 6. Using Step Over, step to the next statement. Note that we are now evaluating statement by statement. Use Step Over
//        again to navigate to the next statement.
// 7. Using Resume, execute to the next breakpoint. Note that we stay in the loop, we want to get out. Disable the breakpoint
//        and Resume.
