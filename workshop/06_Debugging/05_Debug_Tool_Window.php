...
//        in Person.php.
//    The Debug Tool Window shows us frames on the left; which line of code called into which line of code.
//    We can see the Person.php file is called into from the current file. In large applications, this allows walking the
//       execution stack.
...
// 6. From the Debug Tool Window, we can see all variables in the current scope: $fruit, $fruits, $name, $person and the
//       $_SERVER global.
//    Expand the $fruits variable and see which values are in there. We should see three values.
//    Expand the $person variable and see it holds a variable name.
// 7. The variables shown are only the variables that are in scope. If we want to "pin" inspecting a variable, we can add
//        it under Watches on the right.
...
//    From the Debug Tool Window, expand $person and set the name property to your name using the F2 keyboard shortcut.
// 9. Continue execution once (F9) and see on the Console that your name is printed.
// 10. From the Debug Tool Window, use the Evaluate Expression toolbar button (or Alt+F8) to change the name.
//        E.g. evaluate: $person->setName('Stavro').
